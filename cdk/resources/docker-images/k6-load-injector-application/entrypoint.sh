...
##### skip check zip consistency. It always return non valid response code.
# #check zip consistency -> doesnt work as it also returns not valid return code in case of warnings
# unzip -t $script_name
# if [ $? -ne 0 ]; then
#   echo "The provided script zip file is corrupted"
#   exit 1
# fi

# #check if zip file is valid
# if ! unzip -t $script_name > /dev/null; then
#   echo "The provided script zip file is corrupted"
#   exit 1
# fi


echo "The provided script zip file is valid"
...
if [ -n "$SECRET_NAME" ]; then
  ...
  username=$(aws secretsmanager get-secret-value --secret-id $SECRET_NAME --query SecretString --output text --region $AWS_REGION | jq -r '.sapUser')
  password=$(aws secretsmanager get-secret-value --secret-id $SECRET_NAME --query SecretString --output text --region $AWS_REGION | jq -r '.sapPassword')
  sapBaseUrl=$(aws secretsmanager get-secret-value --secret-id $SECRET_NAME --query SecretString --output text --region $AWS_REGION | jq -r '.sapBaseUrl')
  sapClient=$(aws secretsmanager get-secret-value --secret-id $SECRET_NAME --query SecretString --output text --region $AWS_REGION | jq -r '.sapClient')
  
  #check if the url is valid
  if [[ $sapBaseUrl != http* ]]; then
    echo "The provided sapBaseUrl is not valid: $sapBaseUrl"
    exit 1
  fi

  ##create the k6 secret file
  echo "username=$username" > sapK6.secret
  echo "password=$password" >> sapK6.secret
  echo "sapClient=$sapClient" >> sapK6.secret
  echo "credentials file created successfully!"
fi

## run k6 script
echo "starting k6 script: $k6_script_file with parameters: $K6_PARAMETERS"
K6_STATSD_ENABLE_TAGS=true k6 run -e SAP_BASE_URL=$sapBaseUrl --out output-statsd --secret-source=file=sapK6.secret $k6_script_file $K6_PARAMETERS
if [ $? -ne 0 ]; then
